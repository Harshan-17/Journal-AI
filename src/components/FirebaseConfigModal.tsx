...
export const FirebaseConfigModal: React.FC<FirebaseConfigModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="bg-neutral-900/95 border border-neutral-800 rounded-3xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden text-neutral-100 backdrop-blur-xl">
        {/* Header */}
        <div className="p-5 border-b border-neutral-800/80 flex items-center justify-between bg-black/80">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-neutral-100">Firebase & Database Configuration</h3>
              <p className="text-xs text-neutral-400">Manage Cloud Firestore & Google Authentication credentials</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-4 text-xs">
          {isUsingCustom ? (
            <div className="p-3.5 bg-cyan-950/40 border border-cyan-800/60 rounded-2xl flex items-center justify-between">
              ...
              <button
                ...
                className="px-2.5 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 rounded-xl font-semibold transition-colors flex items-center space-x-1 cursor-pointer"
              >
            ...
            <div className="p-3.5 bg-black/70 border border-neutral-800 rounded-2xl flex items-start space-x-2.5 text-neutral-300">
              <AlertTriangle className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold text-neutral-200">Default Project Binding</p>
          ...
          <div className="space-y-1.5">
            <label className="text-neutral-300 font-semibold">Firebase API Key</label>
            <input
              type="text"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              placeholder="AIzaSy..."
              className="w-full bg-black border border-neutral-750 rounded-xl px-3.5 py-2.5 text-xs font-mono text-neutral-200 focus:outline-none focus:border-cyan-500/80 shadow-inner"
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-neutral-300 font-semibold">Project ID</label>
              <input
                type="text"
                value={projectId}
            ...
            <div className="space-y-1.5">
              <label className="text-neutral-300 font-semibold">Auth Domain</label>
              <input
                type="text"
                value={authDomain}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-neutral-300 font-semibold">App ID</label>
              <input
                type="text"
                value={appId}
            ...
            <div className="space-y-1.5">
              <label className="text-neutral-300 font-semibold">Firestore Database ID</label>
              <input
                type="text"
                value={databaseId}
          ...
          <div className="pt-3 flex items-center justify-end space-x-3 border-t border-neutral-800/60">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 rounded-xl font-medium transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
