...
export const SummaryModal: React.FC<SummaryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-950/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="bg-stone-900/95 border border-stone-800 rounded-3xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl shadow-stone-950 overflow-hidden text-stone-100 backdrop-blur-xl">
        {/* Header */}
        <div className="p-5 border-b border-stone-800/80 flex items-center justify-between bg-stone-950/80">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-400 truncate max-w-md">{title || 'Untitled Reflection'}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-4 text-stone-200 text-sm leading-relaxed">
          <div className="prose prose-invert max-w-none prose-headings:text-stone-100 prose-p:text-stone-300 prose-li:text-stone-300 prose-strong:text-cyan-300 prose-code:text-cyan-300">
            <Markdown>{summary}</Markdown>
          </div>
        </div>

        {/* Footer Actions */}
        <div className="p-4 border-t border-stone-800/80 flex items-center justify-between bg-stone-950/80">
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={handleCopy}
              className="flex items-center space-x-1.5 px-3 py-1.5 text-xs font-semibold text-stone-300 hover:text-stone-100 bg-stone-850 hover:bg-stone-800 border border-stone-750 rounded-xl transition-all active:scale-95 cursor-pointer"
            >
